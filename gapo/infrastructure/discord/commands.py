...
def setup_commands(bot: commands.Bot, coach_service, capture_service, config_service):
    ...
    async def coach_start(interaction: discord.Interaction, channel: Optional[discord.VoiceChannel] = None):
        ...
        success = await capture_service.start_coaching(voice_channel.id)
        if success:
    ...
    async def coach_stop(interaction: discord.Interaction):
        await interaction.response.defer()
        await capture_service.stop_coaching()
    ...
    @app_commands.describe(
        ...
        aggressiveness="Agressividade das dicas (1-10)"
    )
    async def coach_config(
        ...
        if event_cooldown is not None:
            config_service.set_event_cooldown(event_cooldown)
            changes.append(f"Event cooldown: {event_cooldown}s")
        if gapo_cooldown is not None:
            config_service.set_gapo_cooldown(gapo_cooldown)
            changes.append(f"Gapo cooldown: {gapo_cooldown}s")
        if aggressiveness is not None:
            config_service.set_aggressiveness(max(1, min(10, aggressiveness)))
            changes.append(f"Agressividade: {aggressiveness}")
